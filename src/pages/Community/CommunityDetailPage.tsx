      ...
      <div className="mx-auto max-w-4xl px-4">
        ...
        <div className="rounded-[2rem] border border-gray-100 bg-white p-8 md:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.02)] mb-8">
          ...
          <div className="mt-6 flex items-center gap-3">
            ...
            <div>
              <p className="text-[15px] font-bold text-gray-800">{post.author}</p>
              <p className="text-[12px] font-medium text-gray-400 mt-0.5">작성자</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-6 text-[14px] font-medium text-gray-400">
              <span className="flex items-center gap-2">
                <Heart className="h-5 w-5 text-red-400" strokeWidth={2} />
                {likes}
              </span>
              <span className="flex items-center gap-2">
                <MessageCircle className="h-5 w-5 text-[#F48171]" strokeWidth={2} />
                {comments.length}
              </span>
            </div>
            ...
            >
              <Heart
                className={hasLiked ? "h-5 w-5 fill-white" : "h-5 w-5"}
                strokeWidth={2}
              />
              {hasLiked ? "좋아요 취소" : "좋아요"}
            </button>
              ...
              <div key={comment.id} className="border-b border-gray-50 pb-5 last:border-0 last:pb-0">
                <div className="mb-2 flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <strong className="text-[14px] text-gray-900">{comment.author}</strong>
                  </div>
                  <span className="text-[12px] font-medium text-gray-400">{comment.date}</span>
                </div>
                <p className="pl-9 text-[14px] leading-relaxed text-gray-600">{comment.content}</p>
              </div>
            ))}
          </div>
