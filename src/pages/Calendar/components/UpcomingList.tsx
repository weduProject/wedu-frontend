...
export default function UpcomingList({ schedules, onScheduleClick }: UpcomingListProps) {
  ...
  const upcomingSchedules = schedules.filter((item) => item.date >= todayString);

  return (
    <BaseCard className="p-5 md:p-6 h-full">
      <h3 className="mb-4 text-base font-bold font-serif text-text">다가오는 일정</h3>

      <div className="flex flex-col gap-3">
        {upcomingSchedules.length === 0 ? (
          <p className="py-8 text-center text-sm text-text-muted">예정된 일정이 없습니다.</p>
        ) : (
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-text-muted">{dayOfWeek}요일 {item.time}</p>
                </div>
              </div>
            );
          })
